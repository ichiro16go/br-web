...
export const Market: React.FC<MarketProps> = ({ recallPiles, onRecall, canRecall, playerPoolCount }) => {
  return (
    <div className="flex flex-col h-full bg-black/50 p-2 border-l border-red-900/30 overflow-y-auto custom-scrollbar">
      <h3 className="text-red-400 font-cinzel text-sm mb-4 text-center border-b border-red-900/30 pb-2">
        Covenant Area
      </h3>
      <div className="flex flex-col gap-6 items-center pb-4">
        {recallPiles.map((pile, index) => {
            if (pile.length === 0) {
                 return (
                     <div key={index} className="w-24 h-36 border border-dashed border-gray-700 rounded flex items-center justify-center">
            ...
            return (
                <div key={index} className="relative group w-full flex justify-center flex-col items-center">
                    ...
                    <div className="mt-1 text-[10px] text-gray-500 bg-black/40 px-2 rounded-full border border-gray-800">
                        Remaining: {remaining}
                    </div>
                </div>
            );
