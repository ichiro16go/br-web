...
export const FieldSection: React.FC<FieldSectionProps> = ({ player, isOpponent, phase, setViewingCard }) => {
    ...
    return (
        <div className="flex-1 flex flex-col p-1 gap-1 min-w-0 h-full maz-w-48 relative">
            <div className={`flex-1 flex flex-col min-h-0 relative ${isOpponent ? 'order-2' : 'order-1'} `}>
                 ...
                 >
                      <div className="absolute top-1 right-1 z-20 flex flex-col items-end pointer-events-none">
                          <div className="bg-black/40 backdrop-blur-sm border border-red-500/20 rounded p-1.5 flex flex-col gap-1 shadow-lg">
                      ...
                      <div className="flex flex-col items-center w-full h-full pt-4 lg:pt-4">
                      {fieldRows.map((rowCards, rowIndex) => (
                            <div 
                                key={rowIndex} 
                                className="flex justify-center -space-x-8 lg:-space-x-12 mb-[-5.5rem] lg:mb-[-7.5rem] last:mb-0" 
                                      ...
                                      return (
                                          <div key={c.id} className="transition-all duration-300 relative transform hover:z-20 hover:-translate-y-4 origin-top" style={{ zIndex: colIndex }}>
                                              <Card 
                                                  card={c} 
                                                  size="lg" 
                                                  className={`${animClass} scale-75 lg:scale-90 shadow-xl`}
            ...
            <div className={`h-24 hidden lg:flex gap-2 ${isOpponent ? 'order-1' : 'order-2'}`}>
                <div className="flex-1">
                    <Zone title="プール (Cards)" className="h-full bg-red-950/10 border-red-900/30" contentClassName="flex items-center justify-start overflow-hidden">
                        <div className="flex -space-x-12 px-4 overflow-x-auto w-full custom-scrollbar py-2 items-center">
