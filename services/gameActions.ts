...
import { 
  ...
  createMadness, createObotsuFragment, createStarterSlash, CRAFT_RECIPES 
...
export const handlePassTurn = (state: GameState, playerId: string): GameState => {
    ...
    if (cloneState.players[opponentKey].hasPassed) {
         // handleResolveBattle はエンジン側で呼び出す（再帰的にReducer呼ぶため）
         // ここではPhase遷移を示すActionを返す設計もできるが、
         // 今回はReducer内でhandleResolveBattleを呼ぶ形にするため、ここではStateを返す
         // ただしEngine側でこれを見て再帰処理する必要がある
         return cloneState; 
    } else {
...
export const handleProcessNextTurnStartEffect = (state: GameState): GameState => {
    ...
    if (desc.includes('手札にあるアーツカードを1枚選ぶ')) {
            ...
            if (state.turnPlayerId === 'p1') {
                ...
                cloneState.pendingTurnStartEffects.shift();
                // 再帰呼び出しはEngine側で行うか、ここでフラグを立てるが、
                // Engineでループ処理を行う形にする
                return cloneState;
            }
    }
...
export const handleResolvePendingAction = (state: GameState, payload: any): GameState => {
        ...
        if (paid) {
            if (player.bloodPool.length >= amount) {
                ...
                if (amount === 1) { 
                    ...
                    cloneState.pendingResolution = { type: 'BURIAL_SEARCH_DECK', cards: [...player.deck] };
                    return cloneState;
                } else if (amount === 7) { 
                    delete cloneState.pendingResolution;
        ...
        if (targetCard) {
            ...
            if (pileIndex !== -1) {
                ...
                if (card) {
                    ...
                    // 想起時効果処理 (再帰なし簡易)
                    // 注: ここでの再帰効果処理は省略または別途resolveFieldEntryEffectsを呼ぶ必要があるが、
                    // pending中にpendingが発生すると複雑化するため、即時効果のみか、ログ出力に留めるのが安全。
                    // 今回はgameActions内で解決関数をインポートしているため呼ぶことは可能。
    ...
    // --- 超克の桜 (Cherry Victory) Actions ---
        ...
        return handleCleanup(cloneState);
    }
    ...
    if (cloneState.pendingTurnStartEffects && cloneState.pendingTurnStartEffects.length > 0) {
        return handleProcessNextTurnStartEffect(cloneState);
    }
